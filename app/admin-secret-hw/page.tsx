...
async function analyzeImageWithClaude(base64:string,mediaType:string,password:string,deviceType?:string):Promise<ClaudeImageAnalysis>{
  const res=await fetch("/api/hw-admin/analyze-image",{method:"POST",headers:{"Content-Type":"application/json","x-admin-password":password},body:JSON.stringify({imageBase64:base64,mediaType,deviceType:deviceType||null})});
  const json=await res.json().catch(()=>({}));
  if(!res.ok)throw new Error(json.error??`Analysis failed (${res.status})`);
  return json as ClaudeImageAnalysis;
}

const C={bg:"#180b2c",surface:"#211038",border:"#341a63",red:"#ff2e9e",gold:"#ffd23f",purple:"#9333ea",textPri:"#f3e8ff",textSec:"#af98cf",textMut:"#5c4a8a",green:"#4ade80",white:"#ffffff"};
...
function Btn({children,onClick,disabled,variant="primary",style}:{children:React.ReactNode;onClick?:()=>void;disabled?:boolean;variant?:"primary"|"ghost"|"danger"|"success";style?:React.CSSProperties;}){
  ...
  const variants:Record<string,React.CSSProperties>={primary:{background:C.red,color:C.white},ghost:{background:"transparent",color:C.textSec,border:`1px solid ${C.border}`},danger:{background:"rgba(255,46,158,0.15)",color:C.red,border:`1px solid ${C.red}`},success:{background:"rgba(74,222,128,0.15)",color:C.green,border:`1px solid ${C.green}`}};
...
function Msg({msg}:{msg:{type:"ok"|"err";text:string}|null}){if(!msg)return null;return<div style={{padding:"10px 14px",marginBottom:"16px",border:`1px solid ${msg.type==="ok"?C.green:C.red}`,color:msg.type==="ok"?C.green:"#ffd080",fontSize:"0.82rem",background:msg.type==="ok"?"rgba(74,222,128,0.08)":"rgba(255,46,158,0.08)"}}>{msg.text}</div>;}
...
function HtmlToolbar({textareaId,value,onChange,password}:{textareaId:string;value:string;onChange:(v:string)=>void;password?:string;}){
  const tags=[{label:"B",wrap:["<strong>","</strong>"]},{label:"I",wrap:["<em>","</em>"]},{label:"H2",wrap:["<h2>","</h2>"]},{label:"H3",wrap:["<h3>","</h3>"]},{label:"P",wrap:["<p>","</p>"]},{label:"UL",wrap:["<ul>\n  <li>","</li>\n</ul>"]},{label:"LI",wrap:["<li>","</li>"]},{label:"A",wrap:['<a href="">','</a>']},{label:"Red",wrap:['<span style="color:#ff2e9e">','</span>']},{label:"Gold",wrap:['<span style="color:#ffd23f">','</span>']},{label:"BQ",wrap:['<blockquote style="border-left:3px solid #ff2e9e;padding:8px 16px;margin:12px 0;font-style:italic;">','</blockquote>']}];
...
function AnalyticsTab({password}:{password:string}){
  ...
  return<div>
    ...
    <Card><p style={eyebrow}>AdSense Approval Checklist</p>{ADSENSE_CHECKLIST.map((item,i)=><div key={i} style={{display:"flex",gap:"10px",padding:"8px 0",borderBottom:`1px solid ${C.border}`,fontSize:"0.82rem"}}><span style={{color:item.done?C.green:C.red,flexShrink:0}}>{item.done?"✅":"☐"}</span><span style={{color:item.done?"rgba(74,222,128,0.8)":C.textSec}}>{item.item}</span></div>)}<div style={{marginTop:"16px",background:"rgba(255,46,158,0.08)",padding:"12px 16px",borderLeft:`3px solid ${C.red}`,fontSize:"0.8rem",color:C.textSec}}>💡 <strong style={{color:C.gold}}>Pro tip:</strong> Publish 20 high-quality blog posts (800+ words) over 6–8 weeks before applying.</div></Card>
  </div>;
}
  ...
  return<div style={{display:"grid",gridTemplateColumns:"240px 1fr",gap:"24px",alignItems:"start"}}>
    <div><Card style={{padding:"0",overflow:"hidden"}}>
      ...
      {PAGE_SLUGS.map(({slug,label})=>{
        ...
        return<button key={slug} onClick={()=>{setUseCustom(false);setSelectedSlug(slug);setMsg(null);}} style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",padding:"10px 16px",background:active?"rgba(255,46,158,0.15)":"transparent",border:"none",borderBottom:`1px solid ${C.border}`,borderLeft:`3px solid ${active?C.red:"transparent"}`,color:active?C.textPri:C.textSec,cursor:"pointer",fontSize:"0.78rem",textAlign:"left"}}>
          <span>{label}</span>
    ...
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"20px",flexWrap:"wrap",gap:"12px"}}>
        ...
        <div style={{display:"flex",gap:"8px"}}>
          ...
          {hasContent(activeSlug)&&<Btn onClick={handleDelete} disabled={deleting} variant="ghost" style={{color:C.red,borderColor:"rgba(255,46,158,0.4)"}}>{deleting?"…":"Delete"}</Btn>}
        </div>
      </div>
      <Msg msg={msg}/>
      {loading?<p style={{color:C.textSec}}>Loading…</p>:<div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
        ...
        <Card style={{padding:"16px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"10px",flexWrap:"wrap",gap:"8px"}}>
            ...
            <div style={{display:"flex",gap:"4px"}}>
              {(["html","preview"] as const).map(m=><button key={m} type="button" onClick={()=>setViewMode(m)} style={{background:viewMode===m?(m==="preview"?C.red:"#341a63"):"transparent",border:`1px solid ${C.border}`,color:viewMode===m?C.white:C.textSec,padding:"4px 12px",cursor:"pointer",fontSize:"0.65rem",fontFamily:"monospace"}}>{m==="preview"?"👁 Preview":"HTML"}</button>)}
            </div>
          </div>
        ...
        <Card style={{padding:"14px 16px",background:"rgba(147,51,234,0.08)",borderColor:"rgba(147,51,234,0.3)"}}>
...
function ImageUploaderTab({password}:{password:string}){
  ...
  async function handleGenerateAll(){if(!file)return;setGenerating(true);setMessage(null);try{const base64=await fileToBase64(file);const result=await analyzeImageWithClaude(base64,file.type,password,deviceType);if(result.title)setTitle(result.title);if(result.slug){setSlug(result.slug.toLowerCase().replace(/[^a-z0-9-]+/g,"-").replace(/^-|-$/g,""));}else if(result.title){setSlug(result.title.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,""));}if(result.htmlCode)setDescription(result.htmlCode);if(result.altText)setAltText(result.altText);if(result.metaDescription)setMetaDescription(result.metaDescription);if(result.tags?.length)setSelectedTags(result.tags);if(file&&result.thumbnailFilename)setFile(renameFile(file,result.thumbnailFilename));if(highResFile&&result.highResFilename)setHighResFile(renameFile(highResFile,result.highResFilename));setMessage({type:"ok",text:"✓ AI generated title, slug, HTML card, alt text, meta description & tags!"});}catch(err){setMessage({type:"err",text:`⚠ AI generation failed: ${(err as Error).message}`});}setGenerating(false);}
  ...
  return<div style={{display:"flex",flexDirection:"column",gap:"20px"}}>
    ...
    <div>
      ...
      <div ref={dropRef} onDragOver={e=>{e.preventDefault();setDragging(true);}} onDragLeave={()=>setDragging(false)} onDrop={onDrop} onClick={()=>fileInputRef.current?.click()} style={{border:`2px dashed ${dragging?C.red:file?C.green:C.border}`,background:dragging?"rgba(255,46,158,0.06)":C.surface,padding:"32px 24px",textAlign:"center",cursor:"pointer",transition:"all 0.2s"}}>
    ...
    <Card style={{padding:"16px",borderColor:highResFile?C.gold:C.border,background:highResFile?"rgba(255,210,63,0.05)":C.surface}}>
    ...
    {file&&<>
      <Card style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:"16px",flexWrap:"wrap",padding:"16px 18px",borderColor:"rgba(255,46,158,0.4)",background:"rgba(255,46,158,0.06)"}}>
      ...
      <Card style={{padding:"16px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"8px",flexWrap:"wrap",gap:"8px"}}>
          ...
          <Btn onClick={async()=>{if(!file)return;setGenerating(true);try{const{altText:at}=await analyzeImageWithClaude(await fileToBase64(file),file.type,password,deviceType);setAltText(at);}catch(err){setMessage({type:"err",text:(err as Error).message});}setGenerating(false);}} disabled={generating} variant="ghost" style={{fontSize:"0.65rem",padding:"5px 14px"}}>{generating?"✨ Analysing…":"✨ AI Generate"}</Btn>
        </div>
        <input value={altText} onChange={e=>setAltText(e.target.value)} placeholder="Dark gothic forest wallpaper with moonlit trees — HD download" style={inp}/>
      </Card>
      <Card style={{padding:"16px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"8px",flexWrap:"wrap",gap:"8px"}}>
      ...
      <Card style={{padding:"14px 18px",borderColor:isAdult?C.red:C.border,background:isAdult?"rgba(255,46,158,0.08)":C.surface}}>
      ...
      <Card style={{padding:"14px 18px",borderColor:commentsEnabled?"rgba(255,210,63,0.5)":C.border,background:commentsEnabled?"rgba(255,210,63,0.06)":C.surface}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:"16px"}}>
          <div>
      ...
      <Card style={{padding:"14px 18px",borderColor:selectedTags.includes("lock-screen-wallpaper")||selectedTags.includes("home-screen-wallpaper")?"rgba(255,210,63,0.5)":C.border,background:selectedTags.includes("lock-screen-wallpaper")||selectedTags.includes("home-screen-wallpaper")?"rgba(255,210,63,0.06)":C.surface}}>
        <div>
          ...
          <div style={{display:"flex",gap:"10px",flexWrap:"wrap"}}>
            <Btn onClick={()=>setSelectedTags(prev=>prev.includes("lock-screen-wallpaper")?prev.filter(t=>t!=="lock-screen-wallpaper"):[...prev,"lock-screen-wallpaper"])} variant={selectedTags.includes("lock-screen-wallpaper")?"success":"ghost"} style={{borderColor:selectedTags.includes("lock-screen-wallpaper")?"#ffd23f":undefined,color:selectedTags.includes("lock-screen-wallpaper")?"#ffd23f":undefined}}>{selectedTags.includes("lock-screen-wallpaper")?"🔒 Lock Screen ON":"Mark as Lock Screen"}</Btn>
            <Btn onClick={()=>setSelectedTags(prev=>prev.includes("home-screen-wallpaper")?prev.filter(t=>t!=="home-screen-wallpaper"):[...prev,"home-screen-wallpaper"])} variant={selectedTags.includes("home-screen-wallpaper")?"success":"ghost"} style={{borderColor:selectedTags.includes("home-screen-wallpaper")?"#ffd23f":undefined,color:selectedTags.includes("home-screen-wallpaper")?"#ffd23f":undefined}}>{selectedTags.includes("home-screen-wallpaper")?"🏠 Home Screen ON":"Mark as Home Screen"}</Btn>
          </div>
        </div>
      </Card>
      <Card style={{padding:"14px 18px",borderColor:isPair?"rgba(236,72,153,0.5)":C.border,background:isPair?"rgba(236,72,153,0.06)":C.surface}}>
        ...
        {isPair&&<div style={{marginTop:"16px",display:"flex",flexDirection:"column",gap:"12px"}}>
          ...
          <div style={{marginTop:"12px",padding:"12px 14px",border:`1px solid ${pairHighResFile?C.gold:C.border}`,background:pairHighResFile?"rgba(255,210,63,0.05)":C.surface}}>
      ...
      <Card style={{padding:"14px 16px",background:"rgba(147,51,234,0.06)",borderColor:"rgba(147,51,234,0.3)"}}>
      ...
      {uploadedUrl&&<Card style={{padding:"14px 16px",borderColor:C.green,background:"rgba(74,222,128,0.08)"}}><p style={{color:C.green,fontSize:"0.8rem",marginBottom:"6px"}}>✓ Uploaded successfully</p><a href={uploadedUrl} target="_blank" rel="noopener noreferrer" style={{color:C.gold,fontSize:"0.75rem",wordBreak:"break-all"}}>{uploadedUrl}</a></Card>}
    </>}
  </div>;
}
...
function BlogTab({password,prefillTitle,prefillLabel,onPrefillUsed}:{password:string;prefillTitle:string;prefillLabel:string;onPrefillUsed:()=>void}){
  ...
  if(mode==="new"||mode==="edit")return<div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px"}}>
      ...
      <Card style={{padding:"16px",borderColor:featImg?"rgba(255,210,63,0.4)":C.border,background:featImg?"rgba(255,210,63,0.04)":C.surface}}>
  ...
  return<div>
    ...
    <Msg msg={msg}/>
    {loading?<p style={{color:C.textSec}}>Loading posts…</p>:posts.length===0?<Card style={{textAlign:"center",padding:"48px"}}><p style={{color:C.textMut,marginBottom:"16px"}}>No posts yet.</p><Btn onClick={openNew}>Write Your First Post</Btn></Card>:<div style={{display:"flex",flexDirection:"column",gap:"8px"}}>{posts.map(p=><Card key={p.slug} style={{padding:"16px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:"16px",flexWrap:"wrap",borderColor:p.published===false?"rgba(255,46,158,0.4)":C.border,background:p.published===false?"rgba(255,46,158,0.04)":C.surface}}><div style={{flex:1,minWidth:0}}><p style={{color:C.textPri,fontWeight:500,marginBottom:"4px",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{p.title}</p><div style={{display:"flex",gap:"12px",fontSize:"0.7rem",color:C.textMut,flexWrap:"wrap"}}><span style={{background:"rgba(147,51,234,0.2)",color:C.purple,padding:"1px 8px"}}>{p.label}</span><span>{new Date(p.createdAt).toLocaleDateString()}</span><span>/blog/{p.slug}</span>{p.published===false&&<span style={{background:"rgba(255,46,158,0.15)",color:C.red,padding:"1px 8px",fontSize:"0.6rem"}}>● HIDDEN</span>}</div></div><div style={{display:"flex",gap:"8px",flexShrink:0}}><Btn onClick={()=>handleTogglePost(p)} variant={p.published===false?"success":"danger"} style={{padding:"7px 14px",fontSize:"0.68rem"}}>{p.published===false?"⬆ Publish":"⬇ Unpublish"}</Btn><Btn onClick={()=>openEdit(p)} variant="ghost" style={{padding:"7px 14px",fontSize:"0.68rem"}}>✏️ Edit</Btn><Btn onClick={()=>handleDelete(p)} disabled={deleting===p.slug} variant="danger" style={{padding:"7px 14px",fontSize:"0.68rem"}}>{deleting===p.slug?"…":"Delete"}</Btn></div></Card>)}</div>}
  </div>;
}
...
function ResidentTagPicker({selectedTags,onToggle,password}:{selectedTags:string[];onToggle:(tag:string)=>void;password:string}){
  ...
  return(
    <div style={{marginTop:"12px",borderTop:"1px solid rgba(168,85,247,0.2)",paddingTop:"12px"}}>
      <p style={{fontFamily:"monospace",fontSize:"0.62rem",letterSpacing:"0.15em",textTransform:"uppercase",color:"#a855f7",marginBottom:"8px"}}>👥 Assign to Resident</p>
      <div style={{display:"flex",flexWrap:"wrap",gap:"6px"}}>
        {residents.map(r=>{const tag=`resident:${r.slug}`;const active=selectedTags.includes(tag);return(
          <button key={r.slug} type="button" onClick={()=>onToggle(tag)} style={{background:active?"rgba(168,85,247,0.25)":"transparent",border:`1px solid ${active?"#a855f7":"rgba(168,85,247,0.3)"}`,color:active?"#c77dff":"rgba(168,85,247,0.7)",padding:"5px 14px",cursor:"pointer",fontSize:"0.68rem",fontFamily:"monospace"}}>
            {active?"✓ ":""}{r.name}
          </button>
        );})}
      </div>
...
function PublishedImagesTab({password}:{password:string}){
  ...
  async function handleAiRegenerate(){if(!editing)return;setAiLoading(true);try{const imgUrl=thumbUrl(editing.r2Key);const{data,mediaType}=await urlToBase64(imgUrl);const result=await analyzeImageWithClaude(data,mediaType,password);if(result.title)setETitle(result.title);if(result.htmlCode)setEDesc(result.htmlCode);if(result.altText)setEAlt(result.altText);if(result.metaDescription)setEMetaDesc(result.metaDescription);if(result.tags?.length)setETags(result.tags);setMsg({type:"ok",text:"✓ AI regenerated title, description, alt text, meta description & tags!"});}catch(err){setMsg({type:"err",text:`⚠ AI failed: ${(err as Error).message}`});}setAiLoading(false);}
  async function handleSave(){if(!editing)return;setSaving(true);const tags=eTags;try{const res=await fetch(`/api/hw-admin/images/${editing.id}`,{method:"PATCH",headers:{"Content-Type":"application/json","x-admin-password":password},body:JSON.stringify({title:eTitle,description:eDesc,altText:eAlt,metaDescription:eMetaDesc||null,tags,isAdult:eAdult,isAvatar:eAvatar,deviceType:eDevice||null})});const json=await res.json().catch(()=>({}));if(!res.ok){setMsg({type:"err",text:json.error??`Save failed (${res.status})`});setSaving(false);return;}setMsg({type:"ok",text:`✓ Saved "${eTitle}"`});setEditing(null);load(page,q);}catch(err){setMsg({type:"err",text:`Network error: ${(err as Error).message}`});}setSaving(false);}
  ...
  return<div>
    ...
    <div style={{display:"flex",gap:"12px",marginBottom:"20px",flexWrap:"wrap",alignItems:"center"}}>
      ...
      <select value={sortBy} onChange={e=>setSortBy(e.target.value as "default"|"views-desc"|"views-asc"|"downloads-desc")} style={{...inp,width:"auto",padding:"10px 12px",cursor:"pointer",color:sortBy!=="default"?C.gold:C.textSec,border:`1px solid ${sortBy!=="default"?"rgba(255,210,63,0.6)":C.border}`}}>
    ...
    {sortBy==="downloads-desc"&&!loadingDownloads&&<Card style={{padding:"10px 14px",marginBottom:"16px",borderColor:"rgba(255,46,158,0.3)",background:"rgba(255,46,158,0.06)"}}>
    ...
    {editing&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",zIndex:1000,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"40px 16px",overflowY:"auto"}}><div style={{background:C.surface,border:`1px solid ${C.border}`,width:"100%",maxWidth:"720px",padding:"28px",fontFamily:"monospace"}}>
      ...
      <div style={{display:"flex",gap:"16px",marginBottom:"20px",alignItems:"flex-start"}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{background:"rgba(255,46,158,0.06)",border:`1px solid rgba(255,46,158,0.25)`,padding:"12px 16px"}}>
      ...
      <div style={{marginBottom:"14px"}}>
        ...
        <input value={eAlt} onChange={e=>setEAlt(e.target.value)} placeholder="Dark gothic forest wallpaper with moonlit trees — HD download" style={inp}/>
      </div>
      <div style={{marginBottom:"14px"}}>
      ...
      <div style={{marginBottom:"20px",display:"flex",alignItems:"center",gap:"12px",flexWrap:"wrap"}}>
        <Btn onClick={()=>setETags(prev=>prev.includes("lock-screen-wallpaper")?prev.filter(t=>t!=="lock-screen-wallpaper"):[...prev,"lock-screen-wallpaper"])} variant={eTags.includes("lock-screen-wallpaper")?"success":"ghost"} style={{padding:"6px 16px",borderColor:eTags.includes("lock-screen-wallpaper")?"#ffd23f":undefined,color:eTags.includes("lock-screen-wallpaper")?"#ffd23f":undefined}}>{eTags.includes("lock-screen-wallpaper")?"🔒 Lock Screen ON":"Lock Screen OFF"}</Btn>
        <Btn onClick={()=>setETags(prev=>prev.includes("home-screen-wallpaper")?prev.filter(t=>t!=="home-screen-wallpaper"):[...prev,"home-screen-wallpaper"])} variant={eTags.includes("home-screen-wallpaper")?"success":"ghost"} style={{padding:"6px 16px",borderColor:eTags.includes("home-screen-wallpaper")?"#ffd23f":undefined,color:eTags.includes("home-screen-wallpaper")?"#ffd23f":undefined}}>{eTags.includes("home-screen-wallpaper")?"🏠 Home Screen ON":"Home Screen OFF"}</Btn>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:"14px",marginBottom:"32px"}}>
      ...
      {[...images].sort((a,b)=>
        ...
        <div style={{position:"relative",aspectRatio:"9/16",background:"#180b2c",overflow:"hidden"}}>
          ...
          {sortBy==="downloads-desc"&&(downloadCounts[img.slug]??0)>0&&<span style={{position:"absolute",top:"6px",left:"6px",background:"rgba(255,46,158,0.9)",color:"#fff",fontSize:"0.55rem",fontFamily:"monospace",fontWeight:900,padding:"2px 6px"}}>#{rank+1}</span>}
          ...
          {!img.altText&&<span style={{position:"absolute",bottom:"6px",left:"6px",background:"rgba(255,46,158,0.85)",color:"#fff",fontSize:"0.5rem",fontFamily:"monospace",padding:"2px 5px"}}>no alt</span>}
        </div>
        <div style={{padding:"10px"}}>
          ...
          <div style={{display:"flex",gap:"6px"}}>
            <button onClick={()=>openEdit(img)} style={{flex:1,background:"rgba(147,51,234,0.1)",border:`1px solid ${C.border}`,color:C.textSec,padding:"6px",cursor:"pointer",fontSize:"0.62rem",fontFamily:"monospace"}}>✏️ Edit</button>
            <button onClick={()=>handleDelete(img)} disabled={deleting===img.id} style={{flex:1,background:"rgba(255,46,158,0.08)",border:"1px solid rgba(255,46,158,0.4)",color:C.red,padding:"6px",cursor:deleting===img.id?"not-allowed":"pointer",fontSize:"0.62rem",fontFamily:"monospace"}}>{deleting===img.id?"…":"🗑 Del"}</button>
          </div>
        </div>
      </div>)}
    </div>}
...
function Manage18Tab({password}:{password:string}){
  ...
  return<div>
    <Card style={{padding:"14px 18px",marginBottom:"24px",borderColor:"rgba(255,46,158,0.5)",background:"rgba(255,46,158,0.06)"}}><p style={{color:C.gold,fontSize:"0.78rem",marginBottom:"4px"}}>⚠ 16+ Content Manager</p><p style={{color:C.textSec,fontSize:"0.72rem"}}>Mark images as 16+ mature themes. Searches by title (partial match).</p></Card>
      ...
      {loading?<p style={{color:C.textSec,padding:"16px",fontSize:"0.8rem"}}>Loading…</p>:
        <div style={{maxHeight:"70vh",overflowY:"auto"}}>
          ...
          {filtered.map(c=>{
            ...
            return<button key={c.slug} onClick={()=>openCollection(c)} style={{display:"flex",alignItems:"center",gap:"10px",width:"100%",padding:"10px 14px",background:active?"rgba(255,46,158,0.15)":"transparent",border:"none",borderBottom:`1px solid ${C.border}`,borderLeft:`3px solid ${active?C.red:"transparent"}`,cursor:"pointer",textAlign:"left"}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{display:"flex",gap:"6px",marginTop:"2px",flexWrap:"wrap"}}>
                  {!c.isPublished&&<p style={{color:C.red,fontSize:"0.55rem",margin:0,background:"rgba(255,46,158,0.15)",padding:"1px 5px"}}>● HIDDEN</p>}
                  {c.rootSlug&&<p style={{color:C.gold,fontSize:"0.55rem",margin:0,background:"rgba(255,210,63,0.15)",padding:"1px 5px"}}>◆ ROOT</p>}
      ...
      :<div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"12px"}}>
          ...
          <div style={{display:"flex",gap:"8px",flexWrap:"wrap"}}>
            ...
            <Btn onClick={handleDeleteCollection} disabled={deleting} variant="danger" style={{background:"transparent",color:C.red,borderColor:"rgba(255,46,158,0.5)"}}>
              {deleting?"…":"🗑 Delete Collection"}
            </Btn>
          </div>
        </div>
        <Msg msg={msg}/>
        <Card style={{padding:"16px",borderColor:selected.thumbnail?C.gold:"rgba(255,210,63,0.3)",background:selected.thumbnail?"rgba(255,210,63,0.04)":C.surface}}>
          ...
          <div
            ...
            style={{border:`2px dashed ${thumbDragging?C.gold:thumbFile?C.green:C.border}`,background:thumbDragging?"rgba(255,210,63,0.06)":C.surface,padding:"24px",textAlign:"center",cursor:"pointer",transition:"all 0.2s",marginBottom:"10px"}}
          >
          ...
          {!thumbFile&&!selected.thumbnail&&(
            <p style={{color:"rgba(255,46,158,0.7)",fontSize:"0.65rem",marginTop:"4px"}}>⚠ No thumbnail — card shows icon placeholder on homepage &amp; obsessions page.</p>
          )}
        </Card>
        <Card style={{padding:"16px"}}>
        ...
        <Card style={{padding:"16px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"10px",flexWrap:"wrap",gap:"8px"}}>
            ...
            <div style={{display:"flex",gap:"4px"}}>
              {(["html","preview"] as const).map(m=><button key={m} type="button" onClick={()=>setDescMode(m)} style={{background:descMode===m?(m==="preview"?C.red:"#341a63"):"transparent",border:`1px solid ${C.border}`,color:descMode===m?C.white:C.textSec,padding:"4px 12px",cursor:"pointer",fontSize:"0.65rem",fontFamily:"monospace"}}>{m==="preview"?"👁 Preview":"HTML"}</button>)}
            </div>
          </div>
          <HtmlToolbar textareaId="coll-desc-ta" value={desc} onChange={setDesc}/>
          {descMode==="html"
            ?<textarea id="coll-desc-ta" value={desc} onChange={e=>setDesc(e.target.value)} rows={14} placeholder={"<h2>About This Collection</h2>\n<p>Describe what makes this collection unique…</p>\n<p>Include keywords: dark wallpapers, download, gothic art, etc.</p>"} spellCheck={false} style={{...inp,resize:"vertical",lineHeight:"1.6",fontFamily:"monospace",fontSize:"0.8rem"}}/>
  ...
  return <div style={{display:"grid",gridTemplateColumns:"220px 1fr",gap:"20px",alignItems:"start"}}>
    <div style={{background:"#0d0d12",border:`1px solid ${C.border}`,borderRadius:"10px",overflow:"hidden",position:"sticky",top:"1rem"}}>
      ...
      {DISTRICTS_LIST.map(d=>{
        ...
        return <button key={d.id} onClick={()=>setActiveId(d.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:"8px",padding:"9px 14px",background:isActive?"rgba(139,47,217,0.15)":"transparent",border:"none",borderLeft:`3px solid ${isActive?d.accent:"transparent"}`,cursor:"pointer",textAlign:"left",transition:"all 0.15s"}}>
        ...
        :images.length>0
          ?<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:"12px"}}>
            {images.map(img=>{
              ...
              return <div key={img.id} onClick={()=>toggle(img.id)} style={{background:"#111",border:`2px solid ${isSel?"#8b2fd9":"#1e1e1e"}`,borderRadius:"8px",overflow:"hidden",cursor:"pointer",position:"relative",boxShadow:isSel?"0 0 0 2px #8b2fd955":"none",transition:"border-color 0.15s"}}>
                ...
                <div style={{position:"absolute",top:"6px",right:"6px",width:"22px",height:"22px",borderRadius:"50%",border:"2px solid #fff",background:isSel?"#8b2fd9":"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.7rem",color:"#fff",fontWeight:700}}>{isSel?"✓":""}</div>
...
function BulkAiTab({password}:{password:string}){
  ...
  async function runOnImage(img:ImageRecord):Promise<void>{
    ...
    try{
      ...
      const res=await fetch(`/api/hw-admin/images/${img.id}`,{method:"PATCH",headers:{"Content-Type":"application/json","x-admin-password":password},body:JSON.stringify({title:result.title,description:result.htmlCode,altText:result.altText,metaDescription:result.metaDescription,tags})});
  ...
  return<div style={{display:"grid",gridTemplateColumns:"280px 1fr",gap:"24px",alignItems:"start"}}>
    <div><Card style={{padding:"0",overflow:"hidden"}}>
      ...
      {loadingColls?<p style={{color:C.textSec,padding:"16px",fontSize:"0.8rem"}}>Loading…</p>:
        <div style={{maxHeight:"70vh",overflowY:"auto"}}>
          {filtered.map(c=>{const active=selectedColl?.slug===c.slug;return<button key={c.slug} onClick={()=>loadCollectionImages(c)} style={{display:"flex",alignItems:"center",gap:"10px",width:"100%",padding:"10px 14px",background:active?"rgba(255,46,158,0.15)":"transparent",border:"none",borderBottom:`1px solid ${C.border}`,borderLeft:`3px solid ${active?C.red:"transparent"}`,cursor:"pointer",textAlign:"left"}}>
            <div style={{flex:1,minWidth:0}}>
              <p style={{color:active?C.textPri:C.textSec,fontSize:"0.78rem",fontWeight:active?600:400,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.title}</p>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"12px"}}>
        ...
        <Btn onClick={handleRunAll} disabled={running||loadingImgs||images.length===0} style={{background:running?"rgba(255,46,158,0.4)":C.red}}>{running?`⏳ Running (${doneCount}/${images.length})…`:`✨ AI Update All ${images.length} Images`}</Btn>
      </div>
      <Card style={{padding:"14px 16px",borderColor:"rgba(255,210,63,0.3)",background:"rgba(255,210,63,0.05)"}}>
        ...
        {images.map(img=>{
          ...
          return<div key={img.id} style={{border:`1px solid ${borderColor}`,background:C.surface,transition:"border-color 0.3s"}}>
            <div style={{position:"relative",aspectRatio:"9/16",background:"#180b2c",overflow:"hidden"}}>
              ...
              {st.state==="done"&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(74,222,128,0.25)",fontSize:"1.4rem"}}>✓</div>}
              {st.state==="err"&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(255,46,158,0.25)",fontSize:"1.4rem"}}>✗</div>}
            </div>
            <div style={{padding:"8px"}}>
              ...
              {st.state==="idle"&&<button onClick={()=>handleRunOne(img)} style={{width:"100%",background:"rgba(147,51,234,0.1)",border:`1px solid ${C.border}`,color:C.textSec,padding:"5px",cursor:"pointer",fontSize:"0.58rem",fontFamily:"monospace"}}>✨ AI Update</button>}
            </div>
          </div>;
        })}
      </div>}
  ...
  return<div style={{display:"grid",gridTemplateColumns:"280px 1fr",gap:"24px",alignItems:"start"}}>
    ...
    {!selectedColl
      ...
      :<div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
        ...
        <Card style={{padding:"14px 16px",borderColor:"rgba(255,210,63,0.3)",background:"rgba(255,210,63,0.04)"}}>
          ...
          :images.length===0
            ...
            :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:"14px"}}>
              {images.map(img=>{
                ...
                const borderColor=st.state==="done"?C.green:st.state==="err"?C.red:already4k?"rgba(74,222,128,0.4)":C.border;
                return<div key={img.id} style={{border:`1px solid ${borderColor}`,background:C.surface,transition:"border-color 0.3s"}}>
                  <div style={{position:"relative",aspectRatio:"9/16",background:"#180b2c",overflow:"hidden"}}>
                    ...
                    {st.state==="done"&&<div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"rgba(74,222,128,0.25)",gap:"4px"}}><div style={{fontSize:"1.6rem"}}>✓</div><p style={{color:C.green,fontSize:"0.6rem",fontFamily:"monospace"}}>Uploaded!</p></div>}
                    {st.state==="err"&&<div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"rgba(255,46,158,0.35)",gap:"4px",padding:"8px"}}><div style={{fontSize:"1.4rem"}}>✗</div><p style={{color:"#ff8080",fontSize:"0.58rem",fontFamily:"monospace",textAlign:"center",wordBreak:"break-word"}}>{st.msg}</p></div>}
                    {(already4k||st.state==="done")&&<span style={{position:"absolute",top:"6px",right:"6px",background:"rgba(74,222,128,0.9)",color:"#fff",fontSize:"0.5rem",fontFamily:"monospace",fontWeight:900,padding:"2px 6px",letterSpacing:"0.05em"}}>✓ 4K</span>}
      ...
      {pinned.length===0
        ...
        :<div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"12px",marginTop:"12px"}}>
          {[1,2,3].map(slot=>{
            ...
            return<div key={slot} style={{border:`1px solid ${img?C.red:C.border}`,padding:"12px",textAlign:"center",background:img?"rgba(255,46,158,0.06)":"rgba(255,255,255,0.01)"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:"6px",maxHeight:"480px",overflowY:"auto"}}>
        {images.map(img=><div key={img.id} style={{display:"flex",alignItems:"center",gap:"12px",padding:"10px",border:`1px solid ${C.border}`,background:"rgba(255,255,255,0.01)"}}>
          ...
          <div style={{display:"flex",gap:"6px",flexShrink:0}}>
            {[1,2,3].map(slot=><button key={slot} onClick={()=>pin(img,slot)} disabled={!!saving} style={{background:"rgba(255,46,158,0.12)",border:`1px solid rgba(255,46,158,0.4)`,color:C.red,padding:"5px 10px",cursor:saving?"not-allowed":"pointer",fontSize:"0.65rem",fontFamily:"monospace",opacity:saving?0.5:1}}>#{slot}</button>)}
          </div>
        </div>)}
      </div>
    </Card>
...
function NukeTab({password}:{password:string}){
  ...
  return<div style={{maxWidth:"560px",display:"flex",flexDirection:"column",gap:"20px"}}>
    <Card style={{padding:"20px",borderColor:C.red,background:"rgba(255,46,158,0.08)"}}>
    ...
    <Msg msg={result}/>
    <Card style={{padding:"16px"}}>
      <label style={lbl}>Type <code style={{color:C.red,background:"rgba(255,46,158,0.15)",padding:"2px 6px"}}>{PHRASE}</code> to unlock</label>
...
function CommentsTab({password}:{password:string}){
  ...
  return<div style={{display:"flex",flexDirection:"column",gap:"20px"}}>
    <Card style={{padding:"14px 18px",borderColor:"rgba(255,210,63,0.4)",background:"rgba(255,210,63,0.05)"}}>
    ...
    {!loading&&shown.map(c=><Card key={c.id} style={{padding:"18px 20px",borderColor:c.status==="pending"?"rgba(255,210,63,0.3)":C.border,background:c.status==="pending"?"rgba(255,210,63,0.04)":C.surface}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"16px",flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"8px",flexWrap:"wrap"}}>
            ...
            <span style={{background:c.status==="pending"?"rgba(255,210,63,0.15)":"rgba(74,222,128,0.15)",border:`1px solid ${c.status==="pending"?C.gold:C.green}`,color:c.status==="pending"?C.gold:C.green,fontSize:"0.6rem",padding:"2px 8px",fontFamily:"monospace",textTransform:"uppercase"}}>{c.status}</span>
...
function LiveWallpapersTab({password}:{password:string}){
  ...
  return<div>
    ...
    {view==="upload"&&<div style={{display:"flex",flexDirection:"column",gap:"20px",maxWidth:"640px"}}>
      <Card style={{borderColor:"rgba(147,51,234,0.3)",background:"rgba(147,51,234,0.04)",padding:"14px 18px"}}><strong style={{color:C.purple}}>🎬 Live Wallpaper Upload</strong><span style={{color:C.textSec,marginLeft:"8px",fontSize:"0.82rem"}}>MP4, H.264, portrait 9:16, under 30MB</span></Card>
      <Msg msg={uploadMsg}/>
    ...
    {view==="manage"&&<div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
      ...
      {items.map(item=><Card key={item.id} style={{padding:"16px 20px"}}>
        <div style={{display:"flex",gap:"16px",alignItems:"flex-start"}}>
          {item.thumbnailUrl?<img src={item.thumbnailUrl} alt={item.title} style={{width:"56px",height:"100px",objectFit:"cover",flexShrink:0,border:`1px solid ${C.border}`}}/>:<div style={{width:"56px",height:"100px",background:"#150a2a",border:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",color:C.textMut,fontSize:"1.4rem",flexShrink:0}}>🎬</div>}
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"6px",flexWrap:"wrap"}}>
              ...
              <span style={{background:item.isPublished?"rgba(74,222,128,0.15)":"rgba(255,46,158,0.15)",border:`1px solid ${item.isPublished?C.green:C.red}`,color:item.isPublished?C.green:C.red,fontSize:"0.58rem",padding:"2px 8px",fontFamily:"monospace",textTransform:"uppercase"}}>{item.isPublished?"Published":"Hidden"}</span>
            </div>
...
function TrafficBar({source,count,max,isTop}:{source:string;count:number;max:number;isTop:boolean}){
  ...
  const color=isTop?C.red:isDark?"#9333ea":C.gold;
...
function VisitorsTab({password}:{password:string}){
  ...
  return<div>
    ...
    <Card style={{marginBottom:"24px"}}>
      ...
      {darkSocialRow&&<div style={{background:"rgba(147,51,234,0.08)",border:`1px solid rgba(147,51,234,0.3)`,padding:"12px 16px",marginBottom:"16px"}}>
...
function ResidentsTab({ password }: { password: string }) {
  ...
  const TOOLBAR_TAGS = [
    ...
    { label: "UL", wrap: ["<ul>\n  <li>", "</li>\n</ul>"] },
    { label: "LI", wrap: ["<li>", "</li>"] },
    { label: "Red",  wrap: ['<span style="color:#ff2e9e">', "</span>"] },
    { label: "Gold", wrap: ['<span style="color:#ffd23f">', "</span>"] },
    { label: "BQ",   wrap: ['<blockquote style="border-left:3px solid #ff2e9e;padding:8px 16px;margin:12px 0;font-style:italic;">', "</blockquote>"] },
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", marginBottom: "6px" }}>
        {TOOLBAR_TAGS.map(({ label, wrap }) => (
          <button key={label} type="button"
            ...
            style={{ background: "rgba(147,51,234,0.15)", border: `1px solid ${C.border}`, color: C.purple, padding: "3px 9px", cursor: "pointer", fontSize: "0.62rem", fontFamily: "monospace" }}>
            {label}
          </button>
        ))}
      </div>
        ...
        : <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {residents.map(r => (
              <Card key={r.id} style={{ display: "flex", gap: "16px", alignItems: "center", padding: "14px 18px" }}>
                {/* Portrait thumbnail */}
                <div style={{ width: "44px", height: "78px", flexShrink: 0, background: "#150a2a", border: `1px solid ${C.border}`, overflow: "hidden", position: "relative" }}>
                  {r.portraitKey
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                    ...
                    <span style={{ background: r.isPublished ? "rgba(74,222,128,0.15)" : "rgba(255,255,255,0.05)", color: r.isPublished ? C.green : C.textMut, padding: "1px 7px", fontSize: "0.58rem", border: `1px solid ${r.isPublished ? C.green : C.border}` }}>
  ...
  if (view === "edit" && selected) return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "16px", flexWrap: "wrap" }}>
        ...
        <span style={{ background: selected.isPublished ? "rgba(74,222,128,0.15)" : "rgba(255,255,255,0.05)", color: selected.isPublished ? C.green : C.textMut, padding: "2px 10px", fontSize: "0.6rem", border: `1px solid ${selected.isPublished ? C.green : C.border}` }}>
      ...
      <Card>
        ...
        <div style={{ display: "flex", gap: "20px", alignItems: "flex-start", flexWrap: "wrap" }}>
          <div style={{ width: "100px", height: "178px", background: "#150a2a", border: `1px solid ${C.border}`, overflow: "hidden", flexShrink: 0, position: "relative" }}>
            {portraitPreview
      ...
      <Card>
        ...
        {storyMode === "html"
          ...
          : <div style={{ background: "#150a2a", border: `1px solid ${C.border}`, padding: "20px", minHeight: "200px", color: C.textPri, lineHeight: 1.8, fontSize: "0.9rem" }} dangerouslySetInnerHTML={{ __html: eStory || "<p style='color:rgba(255,255,255,0.3)'>Nothing written yet…</p>" }} />
        }
      </Card>
      ...
      <Card>
        ...
        {personalityMode === "html"
          ...
          : <div style={{ background: "#150a2a", border: `1px solid ${C.border}`, padding: "20px", minHeight: "150px", color: C.textPri, lineHeight: 1.8, fontSize: "0.9rem" }} dangerouslySetInnerHTML={{ __html: ePersonality || "<p style='color:rgba(255,255,255,0.3)'>Nothing written yet…</p>" }} />
        }
      </Card>

      {/* Wallpapers — tag instructions */}
      <Card style={{ borderColor: "rgba(255,210,63,0.3)", background: "rgba(255,210,63,0.04)" }}>
        ...
        <code style={{ background: "#150a2a", color: C.gold, padding: "8px 14px", display: "inline-block", fontSize: "0.85rem", border: `1px solid ${C.border}`, letterSpacing: "0.05em" }}>
