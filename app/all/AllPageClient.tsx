...
export default function AllPageClient({
  ...
  return (
    <main style={{ minHeight: "100vh", backgroundColor: "var(--bg-primary, #07050f)", color: "var(--text-primary, #ede4ff)" }}>
      ...
      <section style={{ padding: "clamp(40px,6vw,72px) clamp(16px,5vw,72px) 0", maxWidth: "1280px", margin: "0 auto" }}>
        ...
        <p style={{
          ...
          margin: "0 0 32px",
          maxWidth: "560px",
        }}>Browse every wallpaper published on HauntedWallpapers. downloads for iPhone, Android, PC, and desktop devices. Updated regularly.</p>
          ...
          {(["mobile", "desktop"] as DeviceTab[]).map((t) => {
            ...
            return (
              <button key={t} onClick={() => setTab(t)} style={{
                ...
                textTransform: "uppercase",
                padding: "10px 20px",
                border: active ? "1px solid rgba(255,46,158,0.8)" : "1px solid rgba(255,255,255,0.1)",
                background: active ? "rgba(255,46,158,0.12)" : "rgba(255,255,255,0.03)",
                color: active ? "#fff" : "rgba(237,228,255,0.55)",
                boxShadow: active ? "0 0 20px rgba(255,46,158,0.2)" : "none",
                cursor: "pointer",
                ...
                <span style={{
                  fontSize: "0.52rem",
                  color: active ? "rgba(255,255,255,0.5)" : "rgba(237,228,255,0.3)",
                  letterSpacing: "0.1em",
          ...
          <div style={{ display: "flex", justifyContent: "center", padding: "40px 0", gap: "8px" }}>
            {[0, 1, 2].map((i) => (
              <span key={i} style={{
                ...
                borderRadius: "50%", background: "#ff2e9e",
      ...
      <section style={{
        borderTop: "1px solid rgba(255,46,158,0.15)",
        padding: "clamp(48px,7vw,96px) clamp(16px,5vw,72px)",
        background: "#060310",
        position: "relative",
        overflow: "hidden",
      }}>
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse 60% 50% at 50% 100%, rgba(255,46,158,0.08) 0%, transparent 70%)", pointerEvents: "none" }} />
        <div style={{ maxWidth: "680px", margin: "0 auto", position: "relative" }}>
          ...
          <div style={{ fontFamily: "var(--font-cormorant, serif)", fontSize: "clamp(1.05rem,1.8vw,1.25rem)", lineHeight: 1.9, color: "rgba(237,228,255,0.55)", display: "flex", flexDirection: "column", gap: "1.2rem" }}>
            ...
            <p style={{ margin: 0, fontFamily: "var(--font-space, monospace)", fontSize: "0.65rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "rgba(255,46,158,0.7)" }}>
              Updated daily. New nightmares added while you sleep.
            </p>
            <p style={{ margin: 0, fontFamily: "var(--font-cinzel, serif)", fontSize: "clamp(1.1rem,2vw,1.4rem)", color: "#fff3e0", letterSpacing: "0.05em" }}>
...
function MobileCard({ img, priority }: { img: WallpaperItem; priority: boolean }) {
  ...
  return (
    <Link prefetch={false} href={href} className="hw-wall-card" style={{ position: "relative", display: "block", aspectRatio: "9/16", overflow: "hidden", borderRadius: "6px", background: "#0e0d1a", border: "1px solid rgba(255,255,255,0.06)", transition: "border-color 0.25s, box-shadow 0.25s" }}>
      <Image src={img.src} alt={img.title} fill className="hw-wall-card-img" sizes="(max-width:640px) 33vw, (max-width:1024px) 25vw, 17vw" priority={priority} loading={priority ? "eager" : "lazy"} style={{ objectFit: "cover", transition: "transform 0.5s ease" }} />
      {isNew && <span style={{ position: "absolute", top: 6, left: 6, fontFamily: "var(--font-space, monospace)", fontSize: "0.48rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#fff", background: "#4ade80", padding: "2px 5px", borderRadius: "2px", zIndex: 10 }}>NEW</span>}
...
function DesktopCard({ img, priority }: { img: WallpaperItem; priority: boolean }) {
  ...
  return (
    <Link prefetch={false} href={`/pc/${img.slug}`} className="hw-wall-card" style={{ position: "relative", display: "block", aspectRatio: "16/9", overflow: "hidden", borderRadius: "6px", background: "#0e0d1a", border: "1px solid rgba(255,255,255,0.06)", transition: "border-color 0.25s, box-shadow 0.25s" }}>
      <Image src={img.src} alt={img.title} fill className="hw-wall-card-img" sizes="(max-width:768px) 50vw, 33vw" priority={priority} loading={priority ? "eager" : "lazy"} style={{ objectFit: "cover", transition: "transform 0.5s ease" }} />
      {isNew && <span style={{ position: "absolute", top: 8, left: 8, fontFamily: "var(--font-space, monospace)", fontSize: "0.5rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "#fff", background: "#4ade80", padding: "2px 6px", borderRadius: "2px", zIndex: 10 }}>NEW</span>}
