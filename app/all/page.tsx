...
export default async function AllPage() {
  ...
  const [mobileRaw, desktopRaw, totalMobile, totalDesktop] = await Promise.all([
    db.image.findMany({
      where: { isAdult: false,
        isAvatar: false, deviceType: { in: ["IPHONE", "ANDROID"] }, NOT: { tags: { has: "badge-premium" } } },
    ...
    db.image.findMany({
      where: { isAdult: false,
        isAvatar: false, deviceType: "PC", NOT: { tags: { has: "badge-premium" } } },
      orderBy: { createdAt: "desc" },
      take: INITIAL_TAKE,
      select: { id: true, slug: true, title: true, r2Key: true, deviceType: true, tags: true },
    }),
    db.image.count({ where: { isAdult: false,
        isAvatar: false, deviceType: { in: ["IPHONE", "ANDROID"] }, NOT: { tags: { has: "badge-premium" } } } }),
    db.image.count({ where: { isAdult: false,
        isAvatar: false, deviceType: "PC", NOT: { tags: { has: "badge-premium" } } } }),
  ]);
