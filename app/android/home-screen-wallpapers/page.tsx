...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const title = tag
    ? `Dark #${tag} Home Screen Wallpapers for Android${pageLabel} | MR4K WALLS`
    : `Best Android Home Screen Wallpapers 2026 | Dark, Minimal & Icon-Friendly${pageLabel}`;

  const description = tag
    ? `Dark #${tag} home screen wallpapers for Android. Composed to stay readable behind a full grid of app icons. Download instantly.`
    : "Android home screen wallpapers built around your apps and widgets. Dark, minimal backgrounds that make icons pop without the clutter. downloads for every launcher.";
  ...
  return {
    title,
    description,
    keywords: ["Android home screen wallpaper", "home screen background", "minimal Android wallpaper", "dark wallpaper for apps", "launcher wallpaper", "icon-friendly background", "Android wallpaper 2026", "widget wallpaper", tag ?? "dark"].filter(Boolean),
...
export default async function AndroidHomeScreenPage({ searchParams }: PageProps) {
  ...
  const jsonLd = JSON.stringify({
    ...
    "@type": "ItemList",
    name: "Home Screen Wallpapers for Android | MR4K Walls",
    ...
    <main className="min-h-screen" style={{ backgroundColor: "var(--bg-primary)", color: "var(--text-primary)" }}>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-[60px] pt-10 pb-8">
        ...
        <h1 className="font-display text-3xl md:text-4xl font-bold leading-tight">
          <span className="text-[#ffd23f] italic">Home Screen</span> Wallpapers for Android
        ...
        {dbError ? (
          ...
          <>
            ...
            <IphoneImageGrid
              images={images}
              hrefPrefix="/android"
              altSuffix="dark Android home screen wallpaper HD"
