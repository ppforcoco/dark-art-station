// app/android/lock-screen-wallpapers/page.tsx
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  const description = tag
    ? `Dark #${tag} lock screen wallpapers for Android. Composed with open space up top so the clock and notifications stay readable. Download instantly.`
    : "Wallpapers designed around your Android lock screen clock. Dark AMOLED backgrounds, perfectly centered, zero clutter. downloads that look crisp every time you unlock.";

  const canonical = tag
    ? `${siteUrl}/android/lock-screen-wallpapers?tag=${tag}`
    : `${siteUrl}/android/lock-screen-wallpapers`;
...
export default async function AndroidLockScreenPage({ searchParams }: PageProps) {
  const { tag, page: rawPage } = await searchParams;
  const page = Math.max(1, parseInt(rawPage ?? "1", 10) || 1);
  ...
  try {
    ...
    console.error("[android/lock-screen-wallpapers] DB error:", err);
    dbError = true;
  }

  const totalPages = Math.ceil(total / PAGE_SIZE);
  const baseUrl    = tag ? `/android/lock-screen-wallpapers?tag=${encodeURIComponent(tag)}` : "/android/lock-screen-wallpapers";
    ...
    <main className="min-h-screen" style={{ backgroundColor: "var(--bg-primary)", color: "var(--text-primary)" }}>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-[60px] pt-10 pb-8">
        ...
        <h1 className="font-display text-3xl md:text-4xl font-bold leading-tight">
          <span className="text-[#ffd23f] italic">Lock Screen</span> Wallpapers for Android
        ...
        {dbError ? (
          ...
          <>
            ...
            <IphoneImageGrid
              images={images}
              hrefPrefix="/android"
              altSuffix="dark Android lock screen wallpaper HD"
