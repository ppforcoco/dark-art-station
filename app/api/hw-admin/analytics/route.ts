...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Top wallpapers (image downloads)
    const topImagesRaw = await db.imageDownload.groupBy({
      by: ["imageId"],
      _count: { imageId: true },
    ...
    for (const row of topImagesRaw) {
      ...
      topWallpapers.push({
        title: img?.title ?? row.imageId,
        downloads: row._count.imageId,
      });
    }

    // Top collections (by download count)
    ...
    for (const row of topCollectionsRaw) {
      ...
      topCollections.push({
        title: col?.title ?? row.collectionId,
        downloads: row._count.collectionId,
      });
    }
    ...
    const recentActivity = recent.map((r: { createdAt: Date; collection?: { title?: string } | null }) => ({
      time: r.createdAt.toLocaleString("en-US", {
        month: "short", day: "numeric",
        hour: "2-digit", minute: "2-digit",
      }),
      title: r.collection?.title ?? "Unknown",
    }));
