  ...
  return (
    <main className="hw-tag">
      ...
      <section className="hw-tag-section">
        {dbError ? (
          <div className="hw-tag-empty">
            <h2 className="hw-tag-empty__title">Something went wrong</h2>
            <p className="hw-tag-empty__sub">Couldn&apos;t load avatars right now. Try again in a second.</p>
          </div>
        ) : avatars.length === 0 ? (
          <div className="hw-tag-empty">
            <h2 className="hw-tag-empty__title">{config.emptyTitle}</h2>
            <p className="hw-tag-empty__sub">{config.emptyBody}</p>
          </div>
        ) : (
          <>
            <p className="hw-tag-count">
              — {avatars.length} {avatars.length === 1 ? "avatar" : "avatars"} ready to use
            </p>
            <div className="hw-tag-grid">
              {avatars.map((avatar, i) => (
                <article key={avatar.id} className="hw-tag-card">
                  <div className="hw-tag-card__img-wrap">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      src={avatar.src}
                      alt={`${avatar.title} — ${config.pillLabel}`}
                      className="hw-tag-card__img"
                      loading={i < 8 ? "eager" : "lazy"}
                      decoding="async"
                      draggable={false}
                    />
                  </div>
                  <div className="hw-tag-card__body">
                    <h2 className="hw-tag-card__title">{avatar.title}</h2>
                    <div className="hw-tag-card__actions">
                      <a
                        href={`/api/download/image/${avatar.id}`}
                        className="hw-tag-card__btn hw-tag-card__btn--dl"
      ...
      <style>{`
        ...
        @media (min-width: 640px) {
          .hw-tag-grid { grid-template-columns: repeat(auto-fill, minmax(175px, 1fr)); }
        }
        @media (min-width: 1024px) {
          .hw-tag-grid { grid-template-columns: repeat(auto-fill, minmax(210px, 1fr)); }
        }
        ...
        .hw-tag-card:hover .hw-tag-card__img { transform: scale(1.03); }

        .hw-tag-card__body { padding: 10px 10px 12px; }
        .hw-tag-card__title {
          font-family: var(--font-space, monospace);
          font-size: 0.7rem;
          font-weight: 600;
        ...
        .hw-tag-card__actions { display: flex; gap: 6px; }
        .hw-tag-card__btn {
          flex: 1;
          font-family: var(--font-space, monospace);
          font-size: 0.58rem;
        ...
        .hw-tag-card__btn--dl:hover {
          background: rgba(192,0,26,0.22);
          border-color: rgba(192,0,26,0.8);
          color: #fff;
        }

        .hw-tag-empty { text-align: center; padding: 80px 24px; }
        .hw-tag-empty__title {
          font-family: var(--font-display, serif);
