...
// Direct DB fetch — no unstable_cache (avoids cached error poisoning in Next 15)
...
async function getRelated(residentTag: string, currentSlug: string) {
  try {
    return await db.image.findMany({
      where: { tags: { has: residentTag }, slug: { not: currentSlug } },
...
export default async function CharacterWallpaperPage({ params }: PageProps) {
  try {
    ...
    const [resident, relatedWallpapers] = await Promise.all([
      getResident(residentSlug),
      getRelated(residentTag, slug),
    ]);
        ...
        <main style={{ backgroundColor: "var(--bg-primary)", color: "var(--text-primary)", colorScheme: "dark", minHeight: "100vh" }}>
              ...
              <div className="char-info">
                ...
                {image!.tags.filter((t) => !t.startsWith("badge-") && !t.startsWith("resident:")).length > 0 && (
                  <div style={{ padding: "12px 0" }}>
                    <p style={{ fontFamily: "var(--font-space, monospace)", fontSize: "0.55rem", letterSpacing: "0.28em", textTransform: "uppercase", color: "rgba(224,224,224,0.3)", marginBottom: "8px" }}>Tags</p>
          ...
          <style>{`
            ...
            @media (min-width: 768px) {
              .char-grid { flex-direction: row; align-items: flex-start; gap: 48px; }
              .char-image-wrap { flex: 0 0 360px; }
              .char-info { flex: 1; position: sticky; top: 100px; }
            }
            @media (min-width: 1024px) { .char-image-wrap { flex: 0 0 440px; } }
            .char-related-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(130px, 1fr)); gap: 10px; }
    ...
    console.error("[CharacterWallpaperPage] unexpected error:", err);
    notFound();
  }
}