...
function PinButton({ preview, seoAlt }: { preview: string; seoAlt: string }) {
  const url = `https://pinterest.com/pin/create/button/?url=https://hauntedwallpapers.com/sets/cyberpunk-gaming-hero&media=${encodeURIComponent(preview)}&description=${encodeURIComponent(seoAlt)}`;
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="cp-pin-btn" aria-label="Pin to Pinterest">
      <PinIcon />
      Save
    </a>
  );
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <section style={{ maxWidth: "1100px", margin: "clamp(48px,7vw,80px) auto clamp(64px,10vw,120px)", padding: "0 clamp(20px,5vw,60px)" }}>
          <div className="cp-cta-block">
            ...
            <p className="cp-cta-block__sub">All 6 wallpapers. Full 4K resolution. Every device covered.</p>
            <div className="cp-cta-block__btns">
              {WALLPAPERS.map((w) => (
                <a key={w.id} href={w.download} download={w.filename} className="cp-cta-btn">{w.label}</a>
              ))}
            </div>
          </div>
        </section>
      ...
      <style>{`
        ...
        .cp-pin-btn {
          ...
          background: #e60023; color: #fff; border: none; border-radius: 2px;
          font-family: var(--font-space, monospace); font-size: 0.52rem; letter-spacing: 0.12em;
          text-transform: uppercase; text-decoration: none; transition: background 0.2s, transform 0.15s; cursor: pointer;
        }
