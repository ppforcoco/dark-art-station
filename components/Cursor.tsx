...
function trustedHtml(html: string): string | TrustedHTML {
  ...
  if (!_hwPolicy) {
    try { _hwPolicy = window.trustedTypes.createPolicy("hw-cur", { createHTML: (s: string) => s }); }
...
export default function Cursor() {
  ...
  useEffect(() => {
    ...
    const onOver = (e: MouseEvent) => {
      ...
      if (t?.closest(LINK_SEL) && !t?.closest(BTN_SEL)) applyState("hand");
    ...
    const onScroll = () => {
      ...
      scrollTimer = setTimeout(() => {
        ...
        if (h?.closest(LINK_SEL) && !h?.closest(BTN_SEL)) applyState("hand");
        else applyState("default");
      }, 180);
    };
