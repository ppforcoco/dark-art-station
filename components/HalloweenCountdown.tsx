...
export default function HalloweenCountdown() {
  ...
  const setTopbarVar = useCallback((px: number) => {
    ...
    // Always include quote bar in total (DarkQuoteBar will override if dismissed)
    const dqbH = parseInt(
      document.documentElement.style.getPropertyValue("--dqb-h") || String(QUOTE_BAR_HEIGHT), 10
    ) || QUOTE_BAR_HEIGHT;
