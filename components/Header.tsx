...
// Deterministic daily rotation — different dark page every day
const DARK_ROUTES = [
...
export default function Header() {
  ...
  return (
    <>
      {/* ── All new CSS injected once at the top ── */}
      <style>{`
        /* ════════════════════════════════════════════
           BLOOD RED THEME
           ════════════════════════════════════════════ */
        ...
        [data-theme="blood"] .newsletter-section::before { background: radial-gradient(ellipse at 50% 100%, rgba(140,0,0,0.22), transparent 70%) !important; }
        ...
        [data-theme="blood"] .footer-col a:hover   { color: #ffd0d0 !important; }
        [data-theme="blood"] .social-link:hover    { color: #cc0000 !important; }
        ...
        [data-theme="blood"] .mobile-link-index    { color: #cc0000 !important; }
        [data-theme="blood"] .mobile-link-label    { color: #ffd0d0 !important; }
        [data-theme="blood"] .mobile-menu-link:hover .mobile-link-arrow { color: #cc0000 !important; }
        [data-theme="blood"] .mobile-menu-divider  { background: rgba(192,0,0,0.2) !important; }
        ...
        [data-theme="blood"] .hw-search-eyebrow::before,
        [data-theme="blood"] .hw-search-eyebrow::after { background: linear-gradient(90deg, transparent, rgba(192,0,0,0.45), transparent) !important; }
        ...
        [data-theme="blood"] .mobile-action-btn    { background: rgba(192,0,0,0.06) !important; border-color: rgba(192,0,0,0.18) !important; }
        [data-theme="blood"] .mobile-action-btn:hover { background: rgba(192,0,0,0.14) !important; border-color: rgba(192,0,0,0.4) !important; }
        ...
        .hw-dropdown {
          ...
          box-shadow: 0 12px 48px rgba(0,0,0,0.75), 0 0 0 1px rgba(0,0,0,0.5);
          z-index: 800;
        ...
        .hw-search-eyebrow {
          ...
          color: #c0001a; text-align: center;
          margin-bottom: 18px;
          display: flex; align-items: center; gap: 14px;
        }
        .hw-search-eyebrow::before,
        .hw-search-eyebrow::after {
        ...
        .hw-search-clear {
          ...
          color: #4a445a; cursor: pointer; flex-shrink: 0;
        ...
        .search-hint-btn:hover {
          background: rgba(192,0,26,0.2); color: #f0ecff;
        ...
        .mobile-action-btn:active { background: rgba(192,0,26,0.14); color: #f0ecff; border-color: rgba(192,0,26,0.4); }
        .mobile-theme-row {
      ...
      <nav className="site-nav">
        ...
        <div className="nav-right-cluster">
          {/* 1. Search */}
          <button
            type="button" className="hw-nav-icon"
          ...
          {/* 2. Dark Route of the Day */}
          <button
            type="button" className="hw-nav-icon"
            onClick={handleDarkRoute}
            title="Dark Route of the Day"
            aria-label="Dark route of the day"
            style={{ touchAction: "manipulation" }}
          >
          ...
          <div className="hw-dropdown-wrap">
            <button
              type="button" className="hw-dropdown-btn"
              onClick={() => { setCursorMenuOpen(p => !p); setThemeMenuOpen(false); }}
              ...
              <div className="hw-dropdown">
                {CURSOR_STYLES.map(c => (
                  <button
                    key={c.id}
                    type="button"
                    className={`hw-dropdown-item${cursorStyle === c.id ? " hw-active" : ""}`}
                    onClick={() => applyCursor(c.id)}
                  >
                    <span className="hw-dot">{c.emoji}</span>
                    <span>{c.label}</span>
                  </button>
                ))}
              </div>
          ...
          <div className="hw-dropdown-wrap">
            <button
              type="button" className="hw-dropdown-btn"
              onClick={() => { setThemeMenuOpen(p => !p); setCursorMenuOpen(false); }}
        ...
        <div className="nav-mobile-controls">
          <button
            type="button" className="btn-hamburger btn-search-mobile"
            onClick={openSearch} aria-label="Search"
            style={{ touchAction: "manipulation" }}
          >
          ...
          <button
            className="btn-hamburger" onClick={toggleMenu}
          ...
          <div className="hw-search-panel">
            ...
            <form onSubmit={handleSearch}>
              <div className="hw-search-form-box">
                <span className="hw-search-icon-wrap">
                  <Search size={20} strokeWidth={1.2} />
                </span>
                <input
                  ref={overlayInputRef}
                  type="text"
                  className="hw-search-big-input"
        ...
        <div className="mobile-menu-panel">
          ...
          <nav className="mobile-menu-nav">
            {NAV_LINKS.map(({ label, href }, i) => (
              <Link
                key={label} href={href} className="mobile-menu-link"
          ...
          <div className="mobile-theme-row" style={{ marginTop: "12px" }}>
            {(["dark", "blood", "light"] as Theme[]).map(t => (
              <button
                key={t}
                type="button"
                className={`mobile-theme-pill${theme === t ? " active" : ""}`}
          ...
          <div className="mobile-cursor-row" style={{ marginTop: "8px" }}>
            {CURSOR_STYLES.map(c => (
              <button
                key={c.id}
                type="button"
                className={`mobile-cursor-pill${cursorStyle === c.id ? " active" : ""}`}
                onClick={() => applyCursor(c.id)}
                title={c.label}
              >
                {c.emoji}
          ...
          <p className="mobile-menu-watermark" style={{ marginTop: "20px" }}>
